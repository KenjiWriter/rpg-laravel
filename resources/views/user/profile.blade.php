...
@include('template.menu')
    <div class="FormContainer centerDiv">
        ...
        <label for="" style="margin-top: 20px; margin-right: 5px;">Luck:</label>{{ $luck}} <blue>||</blue> (<red>Critical chance</red> {{ $crit_chance }}%)<br>
            @else
        <form action="{{ route('stat.add') }}" method="POST">
            @csrf 
            <input type="number" name="amount" id="" style="height: 30px; width: 30px;"><br>
            <label for="" style="margin-top: 20px; margin-right: 5px;">Strength:</label>{{ $strength}}  <input type="submit" name="strength" value="&plus;"> <blue>||</blue> (<red>Physical damage</red> {{ $dmg }}-{{ $dmg_max }})<br>
            <label for="" style="margin-top: 20px; margin-right: 5px;">Intelligence:</label>{{ $intelligence }} <input type="submit" name="intelligence" value="&plus;"> <blue>||</blue> (<deepblue>Magical damage</deepblue> {{ $magic_dmg }}-{{ $magic_dmg_max }}, <deepblue>MP</deepblue> {{ $mana }})<br>
            <label for="" style="margin-top: 20px; margin-right: 5px;">Endurance:</label>{{ $endurance}} <input type="submit" name="endurance" value="&plus;"> <blue>||</blue> (<green>HP</green> {{ $health }}, <yellow>SP</yellow> {{ $stamina }})<br>
            <label for="" style="margin-top: 20px; margin-right: 5px;">Luck:</label>{{ $luck }} <input type="submit" name="luck" value="&plus;"> <blue>||</blue> (<red>Critical chance</red> {{ $crit_chance }}%)<br>
        </form>
        @endif
